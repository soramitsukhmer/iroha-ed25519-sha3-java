...
public abstract class AbstractFieldElementTest {
  ...
  public void squareAndDoubleReturnsCorrectResult() {
    for (int i = 0; i < 1000; i++) {
      ...
      // Assert:
      Assert.assertThat(b2,
          IsEqual.equalTo(b1.multiply(b1).multiply(new BigInteger("2")).mod(getQ())));
    }
  }
  ...
  public void pow22523ReturnsCorrectResult() {
    for (int i = 0; i < 1000; i++) {
      ...
      // Assert:
      Assert.assertThat(b2, IsEqual
          .equalTo(b1.modPow(BigInteger.ONE.shiftLeft(252).subtract(new BigInteger("3")), getQ())));
    }
  }
