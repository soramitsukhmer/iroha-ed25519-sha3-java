...
import jp.co.soramitsu.crypto.ed25519.math.ed25519.Ed25519FieldElement;
...
public class MathUtils {
  ...
  private static final int[] exponents = {0, 26, 26 + 25, 2 * 26 + 25, 2 * 26 + 2 * 25,
      3 * 26 + 2 * 25, 3 * 26 + 3 * 25, 4 * 26 + 3 * 25, 4 * 26 + 4 * 25, 5 * 26 + 4 * 25};
  ...
  private static final BigInteger d = new BigInteger("-121665")
      .multiply(new BigInteger("121666").modInverse(getQ()));
  private static final BigInteger groupOrder = BigInteger.ONE.shiftLeft(252)
  ...
  public static Field getField() {
    return new Field(
        256, // b
        DatatypeConverter
            .parseHexBinary("edffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff7f"),
        // q
  ...
  public static byte[] multiplyAndAddModGroupOrder(final byte[] a, final byte[] b, final byte[] c) {
    final BigInteger result = toBigInteger(a).multiply(toBigInteger(b)).add(toBigInteger(c))
        .mod(groupOrder);
  ...
  public static GroupElement toGroupElement(final byte[] bytes) {
    ...
    final BigInteger tmp = u.multiply(v.pow(7))
        .modPow(BigInteger.ONE.shiftLeft(252).subtract(new BigInteger("3")), getQ()).mod(getQ());
    ...
    return GroupElement.p3(curve, toFieldElement(x), toFieldElement(y), getField().ONE,
   ...
   */
  public static GroupElement toRepresentation(final GroupElement g,
    ...
    switch (g.getRepresentation()) {
      ...
      case P1P1:
        ...
        break;
      case CACHED:
        x = gX.subtract(gY).multiply(gZ.multiply(new BigInteger("2")).modInverse(getQ()))
            .mod(getQ());
  ...
  public static GroupElement addGroupElements(final GroupElement g1, final GroupElement g2) {
    ...
    return GroupElement.p3(g1.getCurve(), toFieldElement(x3), toFieldElement(y3), getField().ONE,
        toFieldElement(t3));
  }
   ...
   */
  public static GroupElement scalarMultiplyGroupElement(final GroupElement g,
