...
public class Ed25519LittleEndianEncodingTest {
  ...
  public void decodeReturnsCorrectFieldElementForSimpleByteArrays() {
    ...
    // Act:
    final Ed25519FieldElement f1 = (Ed25519FieldElement) MathUtils.getField().getEncoding()
        .decode(bytes1);
    final Ed25519FieldElement f2 = (Ed25519FieldElement) MathUtils.getField().getEncoding()
        .decode(bytes2);
  ...
  public void decodeReturnsCorrectFieldElement() {
    for (int i = 0; i < 10000; i++) {
      ...
      // Act:
      final Ed25519FieldElement f = (Ed25519FieldElement) MathUtils.getField().getEncoding()
          .decode(bytes);
  ...
  public void isNegativeReturnsCorrectResult() {
    for (int i = 0; i < 10000; i++) {
      ...
      final boolean isNegative = MathUtils.toBigInteger(t).mod(MathUtils.getQ())
          .mod(new BigInteger("2")).equals(BigInteger.ONE);
      final FieldElement f = new Ed25519FieldElement(MathUtils.getField(), t);

      // Assert:
      Assert.assertThat(MathUtils.getField().getEncoding().isNegative(f),
          IsEqual.equalTo(isNegative));
    }
  }
}
